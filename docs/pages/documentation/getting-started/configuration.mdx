...
-  `FVM_FORWARD_SIGTERM`: Opts into POSIX PID-only cancellation when set to `true` (disabled by default). Set this per invocation only for runners that signal FVM's PID, not its process group. See [Cancelling commands](/documentation/guides/running-flutter#cancelling-commands).
